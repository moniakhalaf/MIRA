...
- **Dashboard** — the third tab is a per-domain dashboard: chips switch between Health, Fitness, Work and Life, each showing live stat tiles (calories/protein left, water, last sleep; weight, weekly workouts and sets; open tasks and focus; habit streaks and spending) with quick actions into every feature of that area
- **Insights** — the fourth tab charts your trends over time from logged data only: weight, calories and protein vs target, sleep, and weekly training volume, each with an honest "not enough data yet" state until there are at least two points
