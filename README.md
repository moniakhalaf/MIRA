...
**Nutrition tracker**
- Daily food log grouped by day (today open, past days collapse to one line), each entry timestamped with the time you ate (editable) and shown in time order
...
- **Healthy recipes (recipe box)** — import a recipe from a link (website / Instagram / TikTok) or pasted text and MIRA reads it via live web search, keeps it only if it's reasonably healthy, and saves it with the source photo, macros per serving, ingredients and method, cited. Tap **"Rebuild library from the web"** and MIRA runs a live web search across every category (breakfast, lunch/dinner, salads, snacks, sweets, drinks) and repopulates the whole box with fresh healthy recipes from trusted sites — each with macros, ingredients, method, a photo where the page exposes one, and a cited source (your own added/imported recipes are kept). Ask MIRA to **discover** healthy recipes for a request ("high-protein GCC dinners under 500 kcal"), add your own with a photo, **browse by category** (Breakfast · Lunch/Dinner · Salads · Snacks · Sweets · Drinks), and **log any recipe as a serving straight into the food log**. Comes seeded with a curated healthy library of ~37 real dishes across every category — shakshuka, ful medames, overnight oats and smoothie bowls; shawarma bowls, kabsa, kofta, grilled fish, dal and falafel; tabbouleh, fattoush and Greek salad; hummus, baba ghanoush, roasted chickpeas and edamame; fruit salad, date energy balls and yogurt-honey; green and protein smoothies and ayran — most with a photo, and every one with full ingredients, method, and honest per-serving macros estimated from standard food composition
