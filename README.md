...
- **Fitness** — evidence-based training and body-composition interpretation from your own InBody log. **Import workouts from any fitness app** (Garmin, Strava, Apple Watch, Polar, Suunto…): upload the exported **TCX/GPX activity file** for exact duration/distance/calories/heart-rate parsed entirely on-device, or a **screenshot of the workout summary** and MIRA reads it — no retyping. (Direct account sync requires a server, so it ships with the hosted cloud version.)
