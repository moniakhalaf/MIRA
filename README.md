...
- **Fitness** — evidence-based training and body-composition interpretation from your own InBody log. **Sync workouts from your fitness apps and watches**: connect **Strava** once (bring-your-own free Strava API app, same own-credentials model as your Anthropic key — no MIRA server, tokens stay on-device) and activities sync automatically, deduplicated, on open or on demand; Garmin, Apple Watch, Polar, Coros and most watches auto-forward to Strava, so one connection covers them. Or bring in a single workout with no account: the exported **TCX/GPX activity file** (parsed exactly, on-device) or a **screenshot of the workout summary** read by MIRA
...
- **Multilingual — pick your language** from 35+ (English, العربية, Español, Français, Deutsch, 中文, 日本語, हिन्दी, Русский, Português, Türkçe, فارسی, اردو, বাংলা, and more) in its own dedicated **Language** card in Settings, and MIRA writes all its replies, suggestions, macro lookups and reviews in it — or leave it on **"Match what I write"** to switch language mid-conversation with you. Choosing a **right-to-left** language (Arabic, Urdu, Persian, Hebrew) flips the whole interface to RTL, and the core chrome — tabs, home chips, greeting, section labels — is translated for Arabic, Spanish and French (other languages keep English menu labels; every AI-generated response is always in your chosen language).
...
- **Insights** — charts your trends over time from logged data only: weight, calories and protein vs target, sleep, and weekly training volume, each with an honest "not enough data yet" state until there are at least two points. A **time-range toggle** (7d · 14d · 30d · 90d · All) reshapes every chart to the window you want
- **Weight-goal projection** — set a goal weight in Settings and Insights shows an honest ETA: it measures your *actual* rate from your own logged weight trend (least-squares slope) and extrapolates to the goal, always labelled an **estimate**, never a promise — and refuses to guess when your weight is holding steady, trending away from the goal, or the data is too thin
...
- **Snap food / label** — point the camera at either. A printed **nutrition label** is read exactly (official, always overrides databases). A **photo of the actual food** is analysed like a nutritionist would: MIRA identifies each dish (cuisine-aware — Gulf/Khaleeji, pan-Arab and international), estimates the visible portion from visual cues, breaks a mixed plate into its components (rice, grilled chicken, salad…) and prices the macros from standard composition data (USDA and regional tables) — always labelled an estimate, reviewed before saving. When a label gives values **per 100 g / 100 ml / serving**, the review card asks **how much you actually had** and rescales the macros live — with a one-tap **"Whole pack"** shortcut (e.g. drank the whole 320 ml can, ate the whole 200 g bag)
...
- **Weekly meal planner + grocery list** — plan Breakfast/Lunch/Dinner/Snack across any week from your recipe box and saved meals, see each day's planned calories and protein against your targets, and log any planned item into the food diary in one tap. **Build a grocery list** that rolls every planned recipe's ingredients into one de-duplicated, tickable shopping checklist you can share or copy
...
- **iOS install hint** — on iPhone/iPad Safari (which has no install prompt), a one-time, dismissible tip on Home explains **Add to Home Screen** so MIRA runs full-screen like a native app
